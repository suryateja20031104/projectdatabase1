...
app.get("/botVerify13", async (request, response) => {
  const getCount = `
  SELECT
    count(*) AS C
  FROM
...
app.get("/kycScore", async (request, response) => {
  const getCount = `
  SELECT
    count(*) AS C
  FROM
  ...
  const details = `
    ...
    WHERE PC_ID='${c}';
  `;
  const getResponse = await db.all(details);
  const result = getResponse[0].PC_Chat_Log.split(" ");
